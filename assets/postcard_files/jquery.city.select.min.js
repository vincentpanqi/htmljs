function citySelectComponent(province_arg, city_arg) {
    ...
    filterProvinceCity(provincesData, allCodeProvince)

    var allCodeCity = [];
    filterProvinceCity(citysData, allCodeCity, true);

    // 根据省的前两位代码，获取下属城市数组
    function getCity(provinceCode_arg, allCodeCity) {
        var provinceCode_arg = provinceCode_arg.substr(0, 2);
        var selectCity = [];
        for (var i = 0; i < allCodeCity.length; i++) {
            var provinceCode = allCodeCity[i][0].substr(0, 2);
            if (provinceCode_arg == provinceCode || provinceCode == 0) {
                selectCity.push(allCodeCity[i]);
            }
    ...
    var newCity = getCity("330000", allCodeCity);


    function fillProvinceHtml(fillClass, province_arr) {
        var $province = $(fillClass);
        for (var i = 0; i < province_arr.length; i++) {
            var $tempItem = $("<option></option>");
            var Nprovince_code = province_arr[i][0];
            var Nprovince_name = province_arr[i][1];

            $tempItem.attr("value",Nprovince_code);
            $tempItem.html(Nprovince_name);
            $province.append($tempItem);
        }
    }


    fillProvinceHtml(".province", allCodeProvince);
    var citySelect = city_arg;
    var provinceSelect = province_arg;

    $(provinceSelect).on("change", function () {
        var index = this.selectedIndex;
        var selectedItem = this.options[index];

        var city_arr = getCity(selectedItem.value, allCodeCity);
        $(".city").html("");
        fillProvinceHtml(citySelect, city_arr);
    });
